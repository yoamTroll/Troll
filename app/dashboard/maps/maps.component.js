...
var service_phenix_1 = require('./service_phenix');
require('rxjs/add/operator/catch');
var MapsComponent = (function () {
    function MapsComponent(mapsAPILoader, ngZone, sensorsService, servicePhenix) {
        this.mapsAPILoader = mapsAPILoader;
        this.ngZone = ngZone;
        this.sensorsService = sensorsService;
        this.servicePhenix = servicePhenix;
    }
    /**
    ...
    **/
    MapsComponent.prototype.computeAreaAround = function (latitude, longitude, precision) {
        return [latitude - precision / 111, latitude + precision / 111, longitude - precision / 76, longitude + precision / 76];
    };
    MapsComponent.prototype.updateDashboardStore = function (device) {
    ...
    */
    MapsComponent.prototype.getDataObjenious = function (map, infoWindow) {
        ...
        this.sensorsService.getDataObjenious().map(function (devices) { return devices.json(); }).subscribe(function (devices) {
            ...
            _this.id_label = id_label;
            //remove sensors where no data is avalaible
            identifiers = _this.removeElements(identifiers, sensors_off);
            var mapping_ids = "http://localhost:8080/association/" + identifiers.sort().toString() + "?format=jsonp&jsonp=JSONP_CALLBACK";
            console.log(mapping_ids);
            Observable_1.Observable.forkJoin(_this.sensorsService.requestUrl(mapping_ids).map(function (data) { return data.json(); })).subscribe(function (data) {
                _this.phenix_sscc = data[0];
                console.log(data[0]);
            });
            for (var i = 0; i <= Math.trunc(identifiers.length / 100); i++) {
            ...
            Observable_1.Observable.forkJoin(observableBatch).subscribe(function (data) {
                ...
                for (var i = 0; i < observableBatch.length; i++) {
                    ...
                    for (var sensor in sensors) {
                        ...
                        var label = _this.id_label[sensors[sensor].id];
                        // create a  marker on the map
                        var marker = _this.setMarker(label, sensors[sensor].lat, sensors[sensor].lng, map, infoWindow);
                        markers.push(marker);
                    }
                }
                _this.markers = markers;
                _this.devices = tmp;
                for (var dev in _this.devices) {
                    var pos = identifiers.indexOf(_this.devices[dev].id);
                    _this.devices[dev].status = _this.phenix_sscc[pos];
                    _this.devices[dev].label = _this.id_label[_this.devices[dev].id];
                }
    ...
    MapsComponent.prototype.convertDateUtc = function (phenix_date, hour) {
    ...
    MapsComponent.prototype.onEnterSscc = function (value) {
        ...
        if (!value) {
            ...
            this.servicePhenix.getPhenixData(value).map(function (data) { return data.json(); }).subscribe(function (data) {
                data.shippingDatetime = _this.convertDateUtc(data.shippingDatetime, true);
                data.delDatetime = _this.convertDateUtc(data.delDatetime);
                _this.phenix_data = data;
                var devices = _this.devices.filter(function (device) { return device.status.toLocaleLowerCase().indexOf(value) > -1; });
                //var marker = this.markers.filter(marker => marker.title.toLocaleLowerCase().indexOf("sensor_"+value) > -1);
                _this.map.setCenter({ lat: parseFloat(devices[0].lat), lng: parseFloat(devices[0].lng) });
                _this.map.setZoom(12);
            });
        }
    };
    MapsComponent.prototype.localiser = function (id) {
        var devices = this.devices.filter(function (device) { return device.id.toLocaleLowerCase().indexOf(id) > -1; });
        if (devices.length < 1) {
  ...
  this.map.setCenter({lat: parseFloat(devices[0].lat), lng: parseFloat(devices[0].lng)});
  this.map.setZoom(12);**/
    MapsComponent.prototype.onEnterStore = function (value) {
        var _this = this;
        if (!value) {
            this.stores = null;
        }
            ...
            var stores = [];
            this.servicePhenix.getPhenixDataByStore(value).map(function (data) { return data.json(); }).subscribe(function (data) {
                console.log(data);
                for (var _i = 0, data_2 = data; _i < data_2.length; _i++) {
                    var store = data_2[_i];
                    var obj = _this.convertArrayIntoObject(store);
                    obj.label = _this.id_label[obj[0]];
                    obj[6] = _this.convertDateUtc(obj[6], true);
                    obj[7] = _this.convertDateUtc(obj[7]);
                    stores.push(obj);
                }
                _this.stores = stores;
                console.log(stores);
    ...
    MapsComponent.prototype.onEnterTour = function (value) {
        var _this = this;
        if (!value) {
            this.stores = null;
        }
            ...
            var stores = [];
            this.servicePhenix.getPhenixDataByTour(value).map(function (data) { return data.json(); }).subscribe(function (data) {
                console.log(data);
    ...
    MapsComponent.prototype.convertArrayIntoObject = function (array) {
    ...
    MapsComponent.prototype.toggleDevicesMap = function () {
    ...
    MapsComponent = __decorate([
        core_1.Component({
            ...
            styles: ["\n    .mappy {\n      margin-top:90px; \n      margin-left: 35px;\n    }"],
            providers: [core_2.MapsAPILoader, sensors_service_1.SensorsService, service_phenix_1.servicePhenix],
            import: [
        ...
        __metadata('design:paramtypes', [core_2.MapsAPILoader, core_1.NgZone, sensors_service_1.SensorsService, service_phenix_1.servicePhenix])
    ], MapsComponent);
