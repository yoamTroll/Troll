...
import { servicePhenix } from './service_phenix';


import { Device, Phenix, Map, Store } from './device';
...
@Component({
    ...
    providers: [MapsAPILoader, SensorsService, servicePhenix],
    import: [
...
export class MapsComponent implements OnInit {
    ...
    public map: any;
    public heatmap: any;
    public warehouseMap: any;
    public phenix_data: Phenix;
    public stores: Store[];
    public store_data: any;
    public phenix_sscc:any;
    public id_label:any;
    public markers:any;
    ...
    constructor(
      ...
      private servicePhenix:servicePhenix
    ) {}
    ...
    */
    ...
    removeElements(arrayOrigin, elements){
    ...
    * @returns void
    */
    getDataObjenious2(map, infoWindow) {
      var identifiers: string[] = [];
      //no data available
      var sensors_off = ['23362423066984449'];
      var markers = []; 
      this.sensorsService.getDataObjenious().map(data => data.json()).subscribe(
        (data) => {
          ...
          identifiers = this.removeElements(identifiers, sensors_off);
          var ids = [];
          for (var i = 0; i <= Math.trunc(identifiers.length/100); i++) {
            ids.push(identifiers.slice(i*100,(i+1)*100));
          }
          let url = "http://localhost:8080/objenious/devices/states/"+ids[0].toString()+"?format=jsonp&jsonp=JSONP_CALLBACK";
          console.log(url);

          this.sensorsService.requestUrl(url).map(data => data.json()).subscribe((data) => {
            this.devices = data.states;
            for (var sensor in this.devices) {
              var marker = this.setMarker(this.devices[sensor].id, this.devices[sensor].lat, 
                this.devices[sensor].lng, map, infoWindow);
              markers.push(marker);
            }
            console.log("first",markers.length);
            let url_bis = "http://localhost:8080/objenious/devices/states/"+ids[1].toString()+"?format=jsonp&jsonp=JSONP_CALLBACK";
            this.addMarker(url_bis, map, infoWindow, markers);
    ...
    addMarker(url, map, infoWindow, markers){
      console.log(url);
      this.sensorsService.requestUrl(url).map(data => data.json()).subscribe((data) => {
        var devices = data.states;
        for (var sensor in devices) {
          var marker = this.setMarker(devices[sensor].id, devices[sensor].lat, 
            devices[sensor].lng, map, infoWindow);
          markers.push(marker);
        }
    ...
    */
    ...
    /**
    * compute the square area around a point
    * with a precision 
    * @param latitude
    * @param longitude
    * @param precision
    * @return square with limits 
    **/
    computeAreaAround(latitude, longitude, precision){
      return [latitude - precision/111,latitude + precision/111,longitude - precision/76,longitude + precision/76];
    }


    updateDashboardStore(device){
      var latitude = device.lat;
      var longitude = device.lng;
      var tab = this.computeAreaAround(latitude, longitude, 50); //By default, 50m of precision
    ...
    * @param infoWindow 
    */
    getDataObjenious(map, infoWindow){
      ...
      var ids = [];
      var tmp = [];
      this.sensorsService.getDataObjenious().map(devices => devices.json()).subscribe(
        (devices) => {
          for (let device of devices){
            //remove doublons
            if(identifiers.indexOf(device.id) ==-1) {
              identifiers.push(device.id);
              if (sensors_off.indexOf(device.id) == -1){
                id_label[device.id] = device.label;
              }
            }
          }
          ...
          identifiers = this.removeElements(identifiers, sensors_off);
          let mapping_ids = "http://localhost:8080/association/"+identifiers.sort().toString()+"?format=jsonp&jsonp=JSONP_CALLBACK";
          console.log(mapping_ids);
          
          Observable.forkJoin(this.sensorsService.requestUrl(mapping_ids).map(data=>data.json())).subscribe(data => {
            this.phenix_sscc = data[0];
            console.log(data[0]);
          });
          for (var i = 0; i <= Math.trunc(identifiers.length/100); i++) {
            // API Objenious can not support a request with more than 100 parameters
            ids.push(identifiers.slice(i*100,(i+1)*100));
            let url = "http://localhost:8080/objenious/devices/states/"+ids[i].toString()+"?format=jsonp&jsonp=JSONP_CALLBACK";
            // store urls to call
            urls.push(url);
          }
          let observableBatch = [];
          for (let url of urls){
            ...
            observableBatch.push(this.sensorsService.requestUrl(url).map(data=>data.json()));
          }
          ...
          Observable.forkJoin(observableBatch).subscribe(data => {
            ...
            for (var i = 0; i < observableBatch.length; i++){
              ...
              for (var sensor in sensors) {
                ...
                var label = this.id_label[sensors[sensor].id];
                // create a  marker on the map
                var marker = this.setMarker(label, sensors[sensor].lat, 
                sensors[sensor].lng, map, infoWindow);
                markers.push(marker);
              }
            }
            ...
            for (var dev in this.devices){
              var pos = identifiers.indexOf(this.devices[dev].id);
              this.devices[dev].status = this.phenix_sscc[pos];
              this.devices[dev].label = this.id_label[this.devices[dev].id];
              //console.log(this.phenix_sscc[dev]);
            }
    ...
    convertDateUtc(phenix_date, hour){
      ...
      var month = phenix_date.substring(4,6);
      var day = phenix_date.substring(6,8);
      if (hour) {
        var hour = phenix_date.substring(9,11);
        var minute = phenix_date.substring(11,13);
        var sec = phenix_date.substring(13,15);
    ...
    onEnterSscc(value: string) {
      if (!value){
        //reinitialize phenix_data
        this.phenix_data=null;
        this.map.setCenter({lat: this.latitude, lng: this.longitude});
        this.map.setZoom(6);
      }
      else{
        this.servicePhenix.getPhenixData(value).map(data=>data.json()).subscribe(data => {
          data.shippingDatetime = this.convertDateUtc(data.shippingDatetime, true);
          data.delDatetime = this.convertDateUtc(data.delDatetime);

          this.phenix_data = data;

          var devices = this.devices.filter(device => device.status.toLocaleLowerCase().indexOf(value) > -1);
    ...
    localiser(id) {
      var devices = this.devices.filter(device => device.id.toLocaleLowerCase().indexOf(id) > -1);
      if(devices.length < 1){
        demo.showNotification('top','right','Données de localisation non disponibles pour ce Roll');
      }else{
        this.map.setCenter({lat: parseFloat(devices[0].lat), lng: parseFloat(devices[0].lng)});
        this.map.setZoom(12);        
      }
        ...
        //let phenix = [];
        //let id_capteur= null;
        let url_phenix = "http://localhost:8080/phenix/exposition/"+value;
        console.log(url_phenix);
        //let url_mapping = "http://localhost:8080/association/sscc/"+value+"?format=jsonp&jsonp=JSONP_CALLBACK";
        //phenix.push(this.sensorsService.requestUrl(url_phenix).map(data=>data.json()));
        //phenix.push(this.sensorsService.requestUrl(url_mapping).map(data=>data.json()));
        Observable.forkJoin(this.sensorsService.requestUrl(url_phenix).map(data=>data.json())).subscribe(data => {
          //this.phenix_data = data[0];
          //id_capteur = data[1];
          console.log(data);
        });
        //this.phenix_data="successfull";
        //zoom on research
        /**var devices = this.devices.filter(device => device.id.toLocaleLowerCase().indexOf(id_capteur) > -1);
        //var marker = this.markers.filter(marker => marker.title.toLocaleLowerCase().indexOf("sensor_"+value) > -1);
        this.map.setCenter({lat: parseFloat(devices[0].lat), lng: parseFloat(devices[0].lng)});
        this.map.setZoom(12);**/

    onEnterStore(value: string){
      if (!value){
        this.stores = null;
      }else{
        var stores = [];
        this.servicePhenix.getPhenixDataByStore(value).map(data=>data.json()).subscribe(data => {
          console.log(data);
          for(let store of data) {
            var obj = this.convertArrayIntoObject(store);
            obj.label = this.id_label[obj[0]];
            obj[6] = this.convertDateUtc(obj[6], true);
            obj[7] = this.convertDateUtc(obj[7]);
            stores.push(obj);
          }
          this.stores=stores;
          console.log(stores);
    ...
    onEnterTour(value: string){
      if (!value){
        this.stores = null;
      }else{
        var stores = [];
        this.servicePhenix.getPhenixDataByTour(value).map(data=>data.json()).subscribe(data => {
          console.log(data);
          for(let store of data) {
            var obj = this.convertArrayIntoObject(store);
            obj.label = this.id_label[obj[0]];
    ...
    toggleDevicesMap() {
      this.map.setMap(this.map.getMap() ? null : this.map);
    }
